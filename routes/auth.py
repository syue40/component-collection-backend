...
from utils.validator import validate_email, validate_pass
...
def signup():
    ...
    if not validate_pass(password):
        res = jsonify(
            {
                "error": "Invalid password format: passwords must contain an \
                    upper and lower case character, a number, and a special character."
            }
        )
        return res
    ...
    try:
        ...
        if result:
            access_token = create_access_token(identity=email)
            res = {"user_added": True, "access_token": access_token}
        else:
            res = {"user_added": False, "error": "Account Already Exists"}
...
def update_profile():
    ...
    conn = get_db()
    result = update_user_details(first_name, last_name, biography, email, conn)
    if result:
...
@jwt_required()
def change_password():
    """Modify the user's password after receiving data from the reset link."""
            ...
            return jsonify(
                {
                    "password_error": "Password must be at least 8 characters, \
                        and contain at least one uppercase, lowercase, numeric, \
...
def reset_password_post():
    """Reset a user's password from an email link."""
    ...
    if not validate_pass(new_password):
        return jsonify(
            {
                "alert": True,
                "message": "Password must be at least 8 characters, and contain at \
                            least one uppercase, lowercase, numeric, and special character.",
            }
        )
    try:
        update_password(sha256_crypt.encrypt(new_password), email, conn)
        return jsonify(
            {
                "message": "Password successfully reset. You can now login with the new password!"
            }
        )
    except Exception:  # pylint: disable=broad-except
